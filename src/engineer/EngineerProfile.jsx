...
import Verify from './VerifyEngineer';
...
function EngineerProfile() {
  ...
  const getData = async(values)=>{
    try { 
      let res =   await AxiosService.get(`/engineer/getengineer/${params.id}`,values)
      console.log(res)
      if(res.status === 200)
      {
          setInitialValues({
              name:res.data.engineer.name,
              email:res.data.engineer.email,
              phonenumber:res.data.engineer.phonenumber,
  ...
  const handleEditUser = async(values)=>{
    console.log(values)
    try {
      let res = await AxiosService.put(`/engineer/editengineer/${params.id}`,values)
