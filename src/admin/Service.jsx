...
function Service() {
  ...
  const getData = async () => {
    try {
      let res = await AxiosService.get(`/admin/getservice/${params.id}`);
      console.log(res)
      console.log(res.data.user[0].service[0][0].brand)
      
  
      if (res.status === 200) {

        const serviceData = res.data.user[0].service[0][0];
         ...
         <div className=" m-0 p-0">
           ...
           <h3 className="service-heading text-center mb-2">Service details</h3>
            ...
            <Form >
                ...
                <Button variant="primary" onClick={() => navigate("/servicerequests")}>
                  Close
                </Button>
                </div>
